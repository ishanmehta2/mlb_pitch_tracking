#!/usr/bin/env python3
"""
extract_segments.py – cut pitch clips from every .mp4 in raw_videos/
and write them to clips/<clip-id>.mp4
"""
...
CLIP_DIR    = Path("clips")
FFMPEG      = "ffmpeg"
RAW_SUFFIX  = ".mp4"          # full-game container
NUM_WORKERS = 8
# -------------------------------------------------------------------------- #

# ------- globals that workers need ---------------------------------------- #
INDEX: dict[str, list] | None = None     # set in _init_pool


def _init_pool(shared_index: dict[str, list]) -> None:
    """Executed once in every worker process – installs INDEX."""
    global INDEX
    INDEX = shared_index
...
def _worker(raw_path: Path) -> None:
    ...
    Cuts every (clip_id, start, dur) belonging to raw_path.
    """
...
def main() -> None:
    ...
    print(f"🎞️  Found {len(raw_files)} {RAW_SUFFIX} files in {RAW_DIR}")

    # ---------- parallel processing --------------------------------------- #
    with mp.Pool(
        processes=NUM_WORKERS,
        initializer=_init_pool,
        initargs=(index,),          # <- give every worker the shared index
    ) as pool:
